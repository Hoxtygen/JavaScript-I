...
inventory.filter(element => {
    return carModels.sort().push(element.car_model);
})
...
let carYears = [];
inventory.forEach(element => {
    return carYears.push(element.car_year);
});
...
let oldCars = [];
inventory.map(oldy => {
    return oldy.car_year < 2000 ? oldCars.push(oldy.car_year) : "";
})
